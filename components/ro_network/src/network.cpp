...
constexpr uint64_t MQTT_MIN_PUBLISH_MS = 500;
constexpr uint64_t MQTT_HEARTBEAT_MS = 60'000;
constexpr char SESSION_COOKIE[] = "ro_session";
constexpr char CSRF_HEADER[] = "X-CSRF-Token";
...
</style></head><body><main><h1>RO Controller</h1><div id="login" class="card"><h3>Login / first setup</h3><label>Admin password <input id="pw" type="password"></label><label>Wi-Fi SSID <input id="ssid"></label><label>Wi-Fi password <input id="wpw" type="password"></label><button onclick="loginNow()">Login</button> <button onclick="setupNow()">First setup</button><div id="msg"></div></div>
...
<script>
let csrf=localStorage.roCsrf||'';
const writeHeaders=()=>csrf?{'X-CSRF-Token':csrf}:{};
async function loginNow(){let r=await fetch('/api/login',{method:'POST',credentials:'same-origin',headers:{'Content-Type':'application/json'},body:JSON.stringify({password:pw.value})});let j=await r.json();if(r.ok){csrf=j.csrf||'';localStorage.roCsrf=csrf;show()}else msg.textContent=j.error||'login failed'}
async function setupNow(){let r=await fetch('/api/setup',{method:'POST',credentials:'same-origin',headers:{'Content-Type':'application/json'},body:JSON.stringify({password:pw.value,ssid:ssid.value,wifi_password:wpw.value,timezone_name:Intl.DateTimeFormat().resolvedOptions().timeZone||'Etc/UTC',utc_offset_minutes:-new Date().getTimezoneOffset()})});let j=await r.json();msg.textContent=j.message||j.error||'saved'}
async function get(p){let r=await fetch(p,{credentials:'same-origin'});if(r.status===401)throw 0;return r.json()}
async function post(p){let r=await fetch(p,{method:'POST',credentials:'same-origin',headers:writeHeaders()});if(r.status===401||r.status===403)throw 0;setTimeout(refresh,250)}
async function refresh(){try{let s=await get('/api/state');state.textContent=s.state;error.textContent=s.error;water.textContent=s.water_available?'OK':'LOW';tank.textContent=s.tank_full?'FULL':'FILLING';pump.textContent=s.pump?'ON':'OFF';flush.textContent=s.flush?'ON':'OFF';wifi.textContent=s.wifi_connected?'ONLINE':'OFFLINE';time.textContent=s.time_source;filters.textContent=JSON.stringify(await get('/api/filters'),null,2);stats.textContent=JSON.stringify(await get('/api/stats'),null,2);let er=await fetch('/api/events',{credentials:'same-origin'});events.textContent=await er.text()}catch(e){app.style.display='none';login.style.display='block'}}
function show(){login.style.display='none';app.style.display='block';refresh();setInterval(refresh,3000)} if(csrf)show();
...
std::string header_value(httpd_req_t* req, const char* name, size_t max_len = 512) {
    if (!req || !name) return {};
    const size_t len = httpd_req_get_hdr_value_len(req, name);
    if (len == 0 || len > max_len) return {};
    std::vector<char> buffer(len + 1);
    if (httpd_req_get_hdr_value_str(req, name, buffer.data(), buffer.size()) != ESP_OK) return {};
    return buffer.data();
}

bool cookie_matches(std::string_view cookies, std::string_view name, std::string_view expected) {
    size_t pos = 0;
    while (pos < cookies.size()) {
        while (pos < cookies.size() && (cookies[pos] == ' ' || cookies[pos] == ';')) ++pos;
        const size_t end = cookies.find(';', pos);
        const std::string_view item = cookies.substr(pos, end == std::string_view::npos ? cookies.size() - pos : end - pos);
        const size_t eq = item.find('=');
        if (eq != std::string_view::npos && item.substr(0, eq) == name && item.substr(eq + 1) == expected) return true;
...
void add_reading(cJSON* root, const char* name, const OptionalReading& reading) {
    cJSON* value = cJSON_AddObjectToObject(root, name);
    cJSON_AddBoolToObject(value, "available", reading.available);
    if (reading.available) cJSON_AddNumberToObject(value, "value", reading.value);
    else cJSON_AddNullToObject(value, "value");
}
...
esp_err_t NetworkManager::init() noexcept {
    ...
    if (mdns_err == ESP_OK || mdns_err == ESP_ERR_INVALID_STATE) {
        ...
        mdns_service_add(nullptr, provisioning_mode_ ? "_http" : "_https", "_tcp", provisioning_mode_ ? 80 : 443, nullptr, 0);
    }
    return ESP_OK;
}
...
bool NetworkManager::authorized(httpd_req_t* req) noexcept {
    if (!config_.admin.configured || !session_valid()) return false;
    const std::string cookies = header_value(req, "Cookie", 1024);
    return cookie_matches(cookies, SESSION_COOKIE, session_token_);
}

bool NetworkManager::origin_allowed(httpd_req_t* req) const noexcept {
    const std::string origin = header_value(req, "Origin", 256);
    if (origin.empty()) return true; // native/CLI clients do not always send Origin
    const std::string host = header_value(req, "Host", 256);
    if (host.empty()) return false;
    return origin == std::string("https://") + host;
}

bool NetworkManager::write_authorized(httpd_req_t* req) noexcept {
    if (!authorized(req) || !origin_allowed(req)) return false;
    const std::string csrf = header_value(req, CSRF_HEADER, 128);
    return !csrf_token_.empty() && csrf == csrf_token_;
}
...
std::string NetworkManager::state_json() const {
    ...
    cJSON_AddBoolToObject(root, "leak_available", config_.hardware.leak_enabled);
    ...
    cJSON_AddBoolToObject(root, "rtc_available", time_.rtc_available());
    ...
    add_reading(root, "feed_flow", s.feed_flow_lpm);
    add_reading(root, "pure_flow", s.pure_flow_lpm);
    add_reading(root, "drain_flow", s.drain_flow_lpm);
    add_reading(root, "feed_tds", s.feed_tds_ppm);
    add_reading(root, "pure_tds", s.pure_tds_ppm);
    add_reading(root, "recovery", s.recovery_percent);

    const std::string filter_json = filters_.status_json(ti.utc_epoch_s, ti.valid);
    cJSON* filter_array = cJSON_ParseWithLength(filter_json.c_str(), filter_json.size());
    if (filter_array) cJSON_AddItemToObject(root, "filters", filter_array);
    else cJSON_AddArrayToObject(root, "filters");
...
esp_err_t NetworkManager::root_handler(httpd_req_t* req) {
    ...
    httpd_resp_set_hdr(req, "Cache-Control", "no-store");
    httpd_resp_set_hdr(req, "Content-Security-Policy", "default-src 'self' 'unsafe-inline'; connect-src 'self'; object-src 'none'; base-uri 'none'");
    httpd_resp_set_hdr(req, "X-Content-Type-Options", "nosniff");
...
esp_err_t NetworkManager::login_handler(httpd_req_t* req) {
    auto* n = self(req); if (!n) return ESP_FAIL;
    if (!n->origin_allowed(req)) return send_error(req, "403 Forbidden", "unexpected origin");
    ...
    cJSON_Delete(root);
    n->session_token_ = svc::Security::random_hex(32);
    n->csrf_token_ = svc::Security::random_hex(24);
    n->session_expires_ms_ = static_cast<uint64_t>(esp_timer_get_time() / 1000) + SESSION_TTL_MS;
    const std::string cookie = std::string(SESSION_COOKIE) + "=" + n->session_token_ +
        "; Path=/; Max-Age=86400; Secure; HttpOnly; SameSite=Strict";
    httpd_resp_set_hdr(req, "Set-Cookie", cookie.c_str());
    return send_json(req, std::string("{\"csrf\":") + json_string(n->csrf_token_) + "}");
}

esp_err_t NetworkManager::setup_handler(httpd_req_t* req) {
    auto* n = self(req); if (!n) return ESP_FAIL;
    if (!n->provisioning_mode_) return send_error(req, "403 Forbidden", "setup is available only in provisioning mode");
    ...
    if (!cJSON_IsString(pw) || std::strlen(pw->valuestring) < 10) {
        cJSON_Delete(root);
        return send_error(req, "400 Bad Request", "password must be at least 10 characters");
    }
    if (!cJSON_IsString(ssid) || std::strlen(ssid->valuestring) == 0) {
        cJSON_Delete(root);
    ...
    cJSON_AddStringToObject(wifi, "ssid", ssid->valuestring);
    ...
    const int offset_min = cJSON_IsNumber(offset) ? offset->valueint : 0;
...
esp_err_t NetworkManager::settings_put_handler(httpd_req_t* req) {
    auto* n = self(req); if (!n) return ESP_FAIL;
    if (!n->write_authorized(req)) return send_error(req, "403 Forbidden", "session, CSRF or origin rejected");
...
esp_err_t NetworkManager::ota_handler(httpd_req_t* req) {
    ...
    if (!n->hooks_.set_ota_hold || !n->hooks_.set_ota_hold(n->hooks_.context, true))
        return send_error(req, "409 Conflict", "unable to enter OTA hold with outputs verified off");
...
void NetworkManager::start_mqtt() noexcept {
    ...
    const std::string configured_root = config_.mqtt.base_topic.empty() ? "ro-controller" : config_.mqtt.base_topic;
    mqtt_root_ = configured_root + "/" + device_id_;
    mqtt_lwt_topic_ = mqtt_root_ + "/availability";
...
void NetworkManager::mqtt_event(void* handler_args, esp_event_base_t, int32_t id, void* event_data) {
    ...
    if (id == MQTT_EVENT_CONNECTED) {
        ...
        const std::string flush_topic = n->mqtt_root_ + "/command/start_flush";
        const std::string reset_topic = n->mqtt_root_ + "/command/reset_error";
        ...
        if (payload != "PRESS" || !n->hooks_.enqueue_command) return;
        if (topic == n->mqtt_root_ + "/command/start_flush")
            n->hooks_.enqueue_command(n->hooks_.context, {CommandType::StartManualFlush, CommandSource::Mqtt});
        else if (topic == n->mqtt_root_ + "/command/reset_error")
...
void NetworkManager::publish_discovery() noexcept {
    ...
    const std::string state_topic = mqtt_root_ + "/state";
    const std::string availability_topic = mqtt_lwt_topic_;

    auto publish_entity = [&](const char* component, const std::string& object_id, const char* name,
                              const char* value_template, const char* unit,
                              const std::string& command_topic, bool enabled) {
        const std::string config_topic = prefix + "/" + component + "/" + device_id_ + "_" + object_id + "/config";
        if (!enabled) {
            esp_mqtt_client_publish(mqtt_, config_topic.c_str(), "", 0, 1, 1);
            return;
        }
        cJSON* root = cJSON_CreateObject();
        cJSON_AddStringToObject(root, "name", name);
        const std::string unique_id = "ro_" + device_id_ + "_" + object_id;
        ...
        const std::string identifier = "ro-controller-" + device_id_;
        cJSON_AddItemToArray(ids, cJSON_CreateString(identifier.c_str()));
        cJSON_AddStringToObject(device, "name", "RO Controller");
        cJSON_AddStringToObject(device, "manufacturer", "DIY");
        cJSON_AddStringToObject(device, "model", "ESP32-S3 RO Controller");
        char* raw = cJSON_PrintUnformatted(root);
        if (raw) {
            esp_mqtt_client_publish(mqtt_, config_topic.c_str(), raw, 0, 1, 1);
            cJSON_free(raw);
        }
        cJSON_Delete(root);
    };

    publish_entity("sensor", "mode", "Mode", "{{ value_json.mode }}", nullptr, {}, true);
    publish_entity("sensor", "state", "State", "{{ value_json.state }}", nullptr, {}, true);
    publish_entity("binary_sensor", "water_available", "Water available", "{{ 'ON' if value_json.water_available else 'OFF' }}", nullptr, {}, true);
    publish_entity("binary_sensor", "tank_full", "Tank full", "{{ 'ON' if value_json.tank_full else 'OFF' }}", nullptr, {}, true);
    publish_entity("binary_sensor", "inlet", "Inlet valve", "{{ 'ON' if value_json.inlet else 'OFF' }}", nullptr, {}, true);
    publish_entity("binary_sensor", "pump", "Pump", "{{ 'ON' if value_json.pump else 'OFF' }}", nullptr, {}, true);
    publish_entity("binary_sensor", "flush", "Flush valve", "{{ 'ON' if value_json.flush else 'OFF' }}", nullptr, {}, true);
    publish_entity("sensor", "pump_runtime", "Pump runtime", "{{ value_json.production_runtime_s }}", "s", {}, true);
    publish_entity("binary_sensor", "leak", "Leak", "{{ 'ON' if value_json.leak_detected else 'OFF' }}", nullptr, {}, config_.hardware.leak_enabled);

    publish_entity("sensor", "feed_flow", "Feed flow", "{{ value_json.feed_flow.value if value_json.feed_flow.available else none }}", "L/min", {}, config_.features.flow_enabled[0]);
    publish_entity("sensor", "pure_flow", "Pure flow", "{{ value_json.pure_flow.value if value_json.pure_flow.available else none }}", "L/min", {}, config_.features.flow_enabled[1]);
    publish_entity("sensor", "drain_flow", "Drain flow", "{{ value_json.drain_flow.value if value_json.drain_flow.available else none }}", "L/min", {}, config_.features.flow_enabled[2]);
    publish_entity("sensor", "feed_tds", "Feed TDS", "{{ value_json.feed_tds.value if value_json.feed_tds.available else none }}", "ppm", {}, config_.features.tds_feed_enabled);
    publish_entity("sensor", "pure_tds", "Pure TDS", "{{ value_json.pure_tds.value if value_json.pure_tds.available else none }}", "ppm", {}, config_.features.tds_pure_enabled);
    publish_entity("sensor", "recovery", "Recovery", "{{ value_json.recovery.value if value_json.recovery.available else none }}", "%", {}, config_.features.recovery_enabled);

    for (size_t i = 0; i < config_.filters.size(); ++i) {
        const std::string object_id = "filter_" + std::to_string(i + 1);
        const std::string name = config_.filters[i].name.empty() ? "Filter " + std::to_string(i + 1) : config_.filters[i].name;
        const std::string tpl = "{{ 'REPLACE' if value_json.filters[" + std::to_string(i) + "].expired else 'OK' }}";
        publish_entity("sensor", object_id, name.c_str(), tpl.c_str(), nullptr, {}, true);
    }

    publish_entity("button", "start_flush", "Start flush", nullptr, nullptr,
                   mqtt_root_ + "/command/start_flush", true);
    publish_entity("button", "reset_error", "Reset error", nullptr, nullptr,
                   mqtt_root_ + "/command/reset_error", true);
}

void NetworkManager::publish_state(bool force) noexcept {
    if (!mqtt_connected_ || !mqtt_) return;
    const uint64_t now = static_cast<uint64_t>(esp_timer_get_time() / 1000);
    if (!force && now - last_publish_ms_ < MQTT_MIN_PUBLISH_MS) return;
    const std::string payload = state_json();
    const bool heartbeat_due = now - last_heartbeat_ms_ >= MQTT_HEARTBEAT_MS;
    if (!force && !heartbeat_due && payload == last_state_payload_) return;

    last_publish_ms_ = now;
    if (force || heartbeat_due) last_heartbeat_ms_ = now;
    last_state_payload_ = payload;
    const std::string topic = mqtt_root_ + "/state";
    esp_mqtt_client_publish(mqtt_, topic.c_str(), payload.c_str(), 0, 1, 0);
}
