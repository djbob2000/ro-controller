...
constexpr uint32_t MAGIC = 0x53544F52U; // "ROTS" little-endian
constexpr uint16_t VERSION = 1;

struct CheckpointRecord {
    uint32_t magic{MAGIC};
    uint16_t version{VERSION};
    uint16_t payload_size{sizeof(DailyStats)};
    DailyStats stats{};
    uint32_t checksum{0};
};
...
uint32_t record_checksum(const CheckpointRecord& record) noexcept {
    return fnv1a(reinterpret_cast<const uint8_t*>(&record.stats), sizeof(record.stats));
}
...
esp_err_t StatisticsService::restore_durable_checkpoint() noexcept {
    ...
    if (in.gcount() != static_cast<std::streamsize>(sizeof(record)) ||
        record.magic != MAGIC || record.version != VERSION ||
