...
struct MqttConfig {
    bool enabled{false}; bool tls{false}; std::string host; uint16_t port{1883};
    std::string username; std::string password; std::string base_topic{"ro-controller"};
...
struct FeatureConfig {
    ...
    bool tds_feed_enabled{false}; bool tds_pure_enabled{false}; bool recovery_enabled{false};
...
private:
    i2c_master_dev_handle_t rtc_dev_{nullptr}; bool ntp_synced_{false}; bool sntp_started_{false}; bool rtc_time_loaded_{false};
    std::string timezone_posix_{"UTC0"};
    esp_err_t read_rtc(int64_t& epoch) noexcept; esp_err_t write_rtc(int64_t epoch) noexcept;
    static void sntp_sync_cb(::timeval* tv); static TimeService* instance_;
};
...
struct DailyStats {
    int64_t day_key{0}; uint64_t pump_runtime_s{0}; uint32_t production_cycles{0}; uint32_t final_flushes{0};
    uint32_t standby_flushes{0}; uint32_t manual_flushes{0}; double feed_l{0}; double pure_l{0}; double drain_l{0};
};
...
public:
    ...
    esp_err_t restore_durable_checkpoint() noexcept;
    esp_err_t persist_durable_checkpoint(size_t history_limit_days = 730) noexcept;
private:
    DailyStats current_{}; int64_t last_observed_epoch_{0};
    void rollover_if_needed(int64_t epoch) noexcept; esp_err_t append_current() noexcept;
};
